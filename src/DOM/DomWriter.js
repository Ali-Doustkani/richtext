 ...
 * @param {HTMLParagraphElement} paragraph The current paragraph that user is typing in.
 * @param {object} model The style model generated by Stylist.
 */
function render(paragraph, model) {
  paragraph.innerHTML = ''
  const queue = createEditorQueue(paragraph)
  model.forEach(item => queue.editParentOf(item).appendChild(children(item)))
  putToDOM(queue, paragraph.parentElement)
}
